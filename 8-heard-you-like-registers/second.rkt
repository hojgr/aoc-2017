...
(define (reg-set idx val registers)
  (let* ([new-reg (dict-set registers idx val)]
        [new-reg-with-highest (dict-set new-reg 'highest-val (max val (find-highest registers)))])
    new-reg-with-highest))
